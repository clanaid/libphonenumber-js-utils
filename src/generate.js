...
module.exports = (async function () {
  try {
    var xmlPhoneNumberMetadata = await fs.readFileSync(
      path.resolve(
        __dirname,
        "../libphonenumber/resources/PhoneNumberMetadata.xml"
      )
    );
    filterxml(
      xmlPhoneNumberMetadata,
      excludedMeta,
      {},
      async function (err, xmlOut) {
        if (err) {
          throw err;
        }

        const doctypeXml = xmlOut.replace(
          "<!DOCTYPE phoneNumberMetadata>",
          doctype
        );
        await fs.writeFileSync(
          path.resolve(__dirname, "./PhoneNumberMetadata.xml"),
          doctypeXml
        );
    ...
    var xmlPhoneNumberMetadataForTesting = await fs.readFileSync(
      path.resolve(
        __dirname,
        "../libphonenumber/resources/PhoneNumberMetadataForTesting.xml"
      )
    );
    filterxml(
      xmlPhoneNumberMetadataForTesting,
      excludedMeta,
      {},
      async function (err, xmlOut) {
        if (err) {
        ...
        await fs.writeFileSync(
          path.resolve(__dirname, "./PhoneNumberMetadataForTesting.xml"),
          xmlOut
        );
